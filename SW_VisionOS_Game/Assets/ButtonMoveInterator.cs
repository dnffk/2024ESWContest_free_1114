...
public class ButtonMoveInteractor : MonoBehaviour
{
    ...
    void Update()
    {
        if (Touch.activeTouches.Count > 0) // 활성화된 터치가 있을 경우
        {
            foreach (var touch in Touch.activeTouches) // 활성화 된 모든 터치 이벤트 순회
            {
                ...
                {
                    GameObject targetObject = touchData.targetObject;

                    if (touch.phase == TouchPhase.Began && targetObject.CompareTag("Movable")) // 터치가 시작되었고, Movable 태그가 있는 경우
                    {
                        selectedObject = targetObject.transform.root.gameObject; // 선택한 오브젝트의 최상위 부모를 할당

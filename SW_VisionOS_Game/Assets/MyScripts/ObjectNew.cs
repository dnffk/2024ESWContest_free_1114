...
public class ObjectSpawner : MonoBehaviour
{
    ...
    private void SpawnBloodObject(GameObject prefab)
    {
        ...
        // 오브젝트의 회전을 벽면에 맞추기
        if (selectedPlane.alignment == PlaneAlignment.Vertical)
        {
            // 벽면에 수직으로 붙도록 회전 설정
            newObject.transform.rotation = Quaternion.LookRotation(-selectedPlane.transform.up, selectedPlane.transform.forward);
        }
        else
        {
            // 바닥면의 경우 평면에 붙도록 아래로 회전
